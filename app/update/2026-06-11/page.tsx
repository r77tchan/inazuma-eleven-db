...
import { ADDED_VOICES, REMOVED_VOICES, type VoiceDiffEntry } from "./voiceDiff";

export const metadata: Metadata = {
  title: "イナイレDB - 2026/06/11 アップデート ボイス変更",
  description:
    "2026年6月11日のアップデート（公式大会アップデート）で追加・削除されたスキルボイス・超次元ボイスのまとめ",
};
...
// ページに表示しないキャラクター
const EXCLUDED_CHARACTER_IDS = new Set(["c11901150", "c11901160"]);

// このスキルだけが増えたキャラは、キャラごとではなく技ごとにまとめて表示する
const GROUP_BY_SKILL_IDS = new Set(["whk00490"]);
...
export default async function Update20260611Page() {
  const addedVoices = ADDED_VOICES.filter(
    (e) => !EXCLUDED_CHARACTER_IDS.has(e.character_id),
  );
  ...
  const [characterMap, skillInfoMap, auraInfoMap] = await Promise.all([
    getCharacterSummariesByIds(allGroups.map((g) => g.character_id)),
    getSkillInfoByIds(
